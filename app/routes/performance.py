...
from app.models import GroupMonthlyPerformance, MonthlyPerformance, Transaction
...
def create_group_performance():
    ...
    user_id = current_user_identity['id']  # Extract the 'id' using square brackets
    
    # Log the user identity and the extracted user_id
    # print('User Identity:', current_user_identity)
    # print('Extracted User ID:', user_id)
    # current_app.logger.debug(f"User Identity: {current_user_identity}")
    # current_app.logger.debug(f"Extracted User ID: {user_id}")
    ...
    try:
        ...
        transaction = Transaction(
            amount=total_paid,
            description=DescriptionType.AMOUNT_CONTRIBUTED,
            user_id=user_id,
            group_id=group_id,
            group_monthly_performance_id=group_performance.id if not existing_performance else existing_performance.id
        )
...
def create_or_update_monthly_performance():
    ...
    try:
        ...
        current_app.logger.error(f"Type Error: {e}")
